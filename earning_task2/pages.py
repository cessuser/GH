...
class taskPage_ET(Page):
    ...
    def vars_for_template(self):
        ...
        self.player.set_shown_num()
        return{
            'round_num': self.round_number,
            'shown_num': self.player.shown_num
        }

    def entry_field_error_message(self, value):
        if value != len(str(self.player.shown_num)):
    ...
    def before_next_page(self):
        ...
        if self.player.entry_field == len(str(self.player.shown_num)):
