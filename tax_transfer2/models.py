...
# set rd1a and rd1b among rd1, .., rd7
    ...
    def set_round_group(self):
        ...
        group800 = [[p.id,p.participant.vars['nt2_points']] for p in self.get_players() if p.participant.vars['income2'] == 800]
        group600 = [[p.id,p.participant.vars['nt2_points']] for p in self.get_players() if p.participant.vars['income2'] == 600]
        group400 = [[p.id,p.participant.vars['nt2_points']] for p in self.get_players() if p.participant.vars['income2'] == 400]
        group200 = [[p.id,p.participant.vars['nt2_points']] for p in self.get_players() if p.participant.vars['income2'] == 200]
        ...
        rd1a_period2 = random.randint(1, 7)
        rd1b_period2 = random.randint(1, 7)
        ...
        for i in range(0, Constants.num_rounds):
            ...
            for p in self.get_players():
                ...
                if p.income2 == 800:
                    p.participant.vars['decgroup_period2'].append(group800_pid.index(p.id)+1)
                if p.income2 == 600:
                    p.participant.vars['decgroup_period2'].append(group600_pid.index(p.id)+1)
                if p.income2 == 400:
                    p.participant.vars['decgroup_period2'].append(group400_pid.index(p.id)+1)
                if p.income2 == 200:
                    p.participant.vars['decgroup_period2'].append(group200_pid.index(p.id)+1)

        nt_dif_median = np.median(nt2_dif_lst)
        for p in self.get_players():
            p.participant.vars['nt2_dif_median'] = nt_dif_median
            print("decgroup order: ", p.participant.vars['decgroup_period2'])
...
# Compare dif of this round: token 400's raw pts - token 200's raw pts > dif median?
    ...
    def find_group_members_info(self, round_var):
        ...
        print("group median: ", self.participant.vars['nt2_dif_median'])
